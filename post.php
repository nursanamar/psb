<?php 
	...
	if (empty($nama) || empty($ttlhr) || empty($tgl) || empty($klmin) || empty($wn) || empty($alamat) || empty($aslsklh) || empty($almasklh)) {
		...
		$walis=mysqli_query($sambung,"INSERT INTO `wali`(`id_csba`, `nama_wali`, `tgl`, `alamat`, `pendidikan`) VALUES('$id_csba','$wali','$tgl_wali','$alamat_wali','$pndkn_wali')") or die(mysqli_error($sambung));
		$ortu=mysqli_query($sambung,"INSERT INTO `orang_tua`(`id_csba` ,`ayah` ,`ibu` ,`tgl_ayah` ,`tgl_ibu` ,`alamat_ayah` ,`alamat_ibu` ,`pendidikan_ayah` ,`pendidikan_ibu`) VALUES('$id_csba','$ayah','$ibu','$tgl_ayah','$tgl_ibu','$alamat_ayah','$alamat_ibu','$pndkn_ayah','$pndkn_ibu')") or die(mysqli_error($sambung));
		$ins=mysqli_query($sambung,"INSERT INTO csba (`id_csba`,`nama`,`tt_lahir`,`tgl_lahir`,`klmin`,`agama`,`warganegara`,`alamat`,`sekolah_asal`,`alamat_sklhasl`,`pass`,`tgl_regist`) VALUES('$id_csba','$nama','$ttlhr','$tgl','$klmin','$agama','$wn','$alamat','$aslsklh','$almasklh','$pasran',CURDATE())");
